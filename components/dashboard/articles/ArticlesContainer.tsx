...
export default function ArticlesContainer() {
  ...
  // Draft
  const { data: draftArticles, isLoading: isFetchingDraftArticles } =
    useRetrieveArticles({
      page: draftPage,
      status: 'draft',
    });

  const { ArticleStats, isLoading: isStatLoading } = useRetrieveArticleStats();
  ...
  const tabs = [
    ...
    },
    {
      title: 'Drafts',
      value: 'drafts',
      count: draftArticles ? draftArticles?.meta.pagination.total_count : 0,
      content: <Drafts draftArticles={draftArticles!} setPage={setDraftPage} />,
    },
  ];
  ...
  return (
    <div className="w-full space-y-6 px-0">
      ...
      {isFetchingCrawledArticles ||
      isFetchingPublishedArticles ||
      isFetchingDraftArticles ? (
        <ArticlesPageSkeleton />
      ) : (
